...
HydraScars (iNFT)  0x03210f64072ceb1040dbdd37b32e7b0caeeae320
```

`HydraScars.mintScar(...)` mints an ERC-721 token whose `tokenURI` returns on-chain JSON: cause + mitigation rule + generation + learner. The "intelligence" of the swarm is therefore portable — any future agent can read the metadata and inherit the defense.

Example minted iNFT (`api_timeout` cause):
